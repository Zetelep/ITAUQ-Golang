...
func (r *PostgresRepository) Update(ctx context.Context, id string, in domain.UpdateInput) (*domain.Administrator, error) {
	...
	query := fmt.Sprintf("UPDATE public.profiles p SET %s FROM auth.users u WHERE u.id = p.id AND p.id = $1 AND p.roles::text = 'administrator' RETURNING %s", setClauses, adminColumns)
